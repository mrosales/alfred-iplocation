...
def setConfig(plist, field)
    val = $config[field]
    if val and val != plist[field]
        plist[field] = val
        puts "Updating info.plist field #{field} to #{val}"
    end
end

task :config do
    info = Plist::parse_xml($config["plist"])
    setConfig(info, "bundleid")
    setConfig(info, "createdby")
    setConfig(info, "description")
    setConfig(info, "name")
    setConfig(info, "description")
    setConfig(info, "webaddress")
